...
- "lesson": an array of 3 to 6 objects (vary the count to match hook number), each with:
  - "icon": a Material Symbols Outlined icon name in snake_case that SPECIFICALLY matches the lesson's title and description. DO NOT reuse the same icon across multiple lessons. Every icon must be intentionally chosen: if the lesson is about passive income → "account_balance" or "payments", if about time → "schedule" or "history", if about networks → "group" or "hub", if about automation → "settings" or "smart_toy", if about content → "article" or "movie", if about royalties → "description" or "receipt". Think deeply about the meaning of each lesson before picking its icon.
